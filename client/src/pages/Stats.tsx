              ...
              topCards.map((card) => (
                <div key={card.id} className="flex items-center space-x-3 p-2 hover:bg-slate-50 rounded">
                  <div className="w-12 h-16 bg-slate-100 rounded overflow-hidden">
                    {card.frontImage ? (
                      <img src={card.frontImage} alt="Card thumbnail" className="object-cover w-full h-full" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 truncate">{card.year} {card.brand} {card.collection} #{card.cardNumber}</p>
                  </div>
                  <div className="text-right">
