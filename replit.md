...
- **Holo Identification + Grading Engine**: Every call to `POST /api/analyze-card-dual-images` auto-runs a single Claude vision analysis that returns BOTH card identification (player, brand, set, collection, year, card number, serial, parallel, variant, CMP code, sport, confidence) AND a PSA-style condition grade — in one API call. Holo produces a PSA-style half-step overall grade (1.0–10.0), four sub-grades (centering front, centering back when provided, corners, edges, surface), per-sub-grade notes, up to three overall takeaways, and a PSA label (GEM MT 10 / MINT 9 / NM-MT 8 / etc.). Grades are attached to the scan response as `data.holo` and rendered by `<HoloGradeCard>` on `/` above the eBay results. All grades are persisted in the `scan_grades` table (indexed by user + created_at), and `GET /api/scan-grades?limit=25` returns the authenticated user's most-recent grades. Implementation: `server/holo/cardGrader.ts` (Claude wrapper), `server/holo/storage.ts` (Drizzle persistence), `client/src/components/HoloGradeCard.tsx` (UI). Holo is fully optional — if `ANTHROPIC_API_KEY` is not set or grading fails, the scan still succeeds and `data.holo = null`. Model is `claude-sonnet-4-5` by default, overridable via the `HOLO_MODEL` env var.

### Holo Identification + Grading
- **Unified analysis**: One Claude vision call returns both identification and grade (see `server/holo/cardGrader.ts#analyzeCard`). This replaces the previous grade-only flow and costs the same as a single grading call.
- **Identification as primary**: Claude is the primary identifier. When identification confidence ≥ 0.7, its fields (parallel, CMP code, serial, exact collection, variant) override the OCR/Gemini output via a shallow merge in `server/routes.ts` — Claude fills in what OCR typically misses. Below the threshold, OCR/Gemini remains authoritative and Holo only contributes the condition grade.
- **Graceful fallback**: If Claude fails entirely (missing key, API error, network issue), the scan falls back to the existing OCR/Gemini + eBay pipeline exactly as before. `data.holo` will be `null`, and every other response field is unchanged.
- **Match percentage**: Claude's identification confidence is surfaced in the UI as a 0–100% "Match" score with a color-coded progress bar. Under 70% shows an amber warning banner so users know to verify fields before saving.
- **Required env var**: `ANTHROPIC_API_KEY` — set as a Replit secret to enable Holo.
- **Optional env var**: `HOLO_MODEL` — override the Claude model (defaults to `claude-sonnet-4-5`).
- **Schema**: `scan_grades` table lives in `shared/schema.ts` with `identification` (jsonb) and `identification_confidence` columns added. Run `npm run db:push` after pulling to apply migrations.
- **Persistence**: Each scan saves the grade + identification together, tied to the user's id (nullable for anonymous scans), so `GET /api/scan-grades` returns a full historical audit trail of what Holo saw.
- **Graded Pricing Breakdown**: When Holo produces an overall grade, the UI renders a three-tier comparable-sales panel (`<GradedPriceBreakdown>`) between the Holo card and the standard eBay results. The panel shows three eBay medians side-by-side: **Raw** (slabbed listings filtered out via `-PSA -BGS -SGC -CGC -graded` negatives), **At grade** (`PSA {predictedGrade}`, where `predictedGrade` is `floor(holo.overall)` clamped to 1–10 — PSA has no half grades), and **Top grade** (`PSA 10`). Powered by `GET /api/ebay-graded-search`, which runs all three searches in parallel through `searchCardValues(..., { gradeKeyword, excludeGraded })`. Empty tiers render "No comps yet — Search eBay" with a deep link to an eBay sold search pre-filled with the card identifiers plus the grade keyword (never AI-invented prices). Grading company is PSA only; other companies (BGS, SGC, CGC) price differently and are out of scope for this panel. Helper: `server/holo/psaGrade.ts#holoOverallToPsaInt` / `psaKeyword`.
